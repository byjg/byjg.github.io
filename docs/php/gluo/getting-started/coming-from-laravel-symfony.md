...
- **Testing.** Functional tests run against the OpenAPI contract, so a response that
  drifts from the documented schema fails the test. See [Testing](../guides/testing).
...
1. [Installation](installation) — create the project
2. [Your first table](first-table) — migration then codegen
3. [Your first endpoint](first-endpoint) — attributes, and the regeneration step
