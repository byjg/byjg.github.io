...
answers on **http://localhost:8080**. See the **[Frontend guide →](guides/frontend)**.
...
2. **[Coming from Laravel, Symfony or Drupal](getting-started/coming-from-laravel-symfony)** – Concept and command mapping, plus the four things that genuinely work differently.
3. **[Create Your First Table](getting-started/first-table)** – Define your first migration and schema.
4. **[Add Fields](getting-started/add-field)** – Safely evolve existing tables.
5. **[Create REST Endpoints](getting-started/first-endpoint)** – Generate REST handlers from your tables.
6. **[Windows Setup](getting-started/windows)** – WSL/Windows-specific checklist.
7. **[Unattended Setup](getting-started/unattended-setup)** – Automate installs for CI/CD pipelines.

### Guides
- **[Frontend (Vite + React)](guides/frontend)** – Run and customize the optional `frontend/` SPA that talks to the API over JWT.
