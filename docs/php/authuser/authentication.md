...
SHA-1 is used for backward compatibility. For new projects, consider implementing a custom password hasher using bcrypt or Argon2. See [Mappers](mappers) for details.
:::

:::tip Enforce Password Strength
To enforce password policies (minimum length, complexity rules, etc.), see [Password Validation](password-validation).
:::
...
3. **Use strong passwords** - see [Password Validation](password-validation)
4. **Set appropriate session timeouts**
5. **Validate and sanitize** all user inputs

## Next Steps

- [Session Context](session-context) - Manage user sessions
- [JWT Tokens](jwt-tokens) - Deep dive into JWT authentication
- [Password Validation](password-validation) - Enforce password policies
