...
`Memory` storage has no file locking. If multiple processes load and save the same file, use an external lock or switch to [RDBMS storage](storage-rdbms).
...
- [Storage: RDBMS](storage-rdbms)
- [Storage backends comparison](../../concepts/storage-backends)
