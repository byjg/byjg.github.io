...
The lexer extracts tokens from the text (see [Lexer concepts](../../concepts/lexer)). Each unique token is counted. A word that appears three times in one training text contributes `count = 3` for that token, not `count = 1`. This means frequently repeated words carry more weight.

## Related

- [Classifying text](classifying)
- [How NaiveBayes works](../../concepts/how-naive-bayes-works)
- [Lexer concepts](../../concepts/lexer)
