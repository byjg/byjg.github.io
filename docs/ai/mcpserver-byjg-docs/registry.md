...
| JetBrains IDEs, Claude Code, Claude Desktop, Codex CLI, Gemini CLI, Cursor | No registry browsing at all: the server is added by hand, see [Connecting a client](clients) |

In short, the registry entry is for **discovery and provenance**, not
installation. [Connecting a client](clients) stays the way to actually use
