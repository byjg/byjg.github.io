...
- This method does not work with containers that use the `--network=host` option. See [limitations](limitations) for details.
:::
...
**Do not** add more than one replica for EasyHAProxy. To understand why, see the [limitations](limitations) page.
:::
