...
| easyhaproxy.[definition].certbot      | (Optional) Generate certificate with certbot. Do not use with `sslcert` parameter. More info [here](../guides/acme).                              | false        | true OR false                                                                                                    |
...
| easyhaproxy.plugin.`{name}`.`{key}` | (optional) Plugin-specific configuration (see [Using Plugins](../guides/plugins)) | *varies*   | See plugin docs             |

For annotation usage examples, see the [Kubernetes getting started guide](../getting-started/kubernetes).
